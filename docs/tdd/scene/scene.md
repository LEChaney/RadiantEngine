...
- `void set_world_transform(NodeHandle node, const glm::mat4& world)` – Sets the node's world transform. Updates the nodes local transform such that `parent_world * local = world`. This function always propagates transforms up to the parent (if dirty) before computing the local transform, ensuring correctness. Its descendants are then marked dirty for propagation.
