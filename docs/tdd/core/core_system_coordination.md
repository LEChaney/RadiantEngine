...
3. **Subsystem Initialization**
   ...
   - **Observer Pattern Registration:**
     ...
     - Example:
       ...
       materialSystem.addObserver(&drawDataManager);
       meshSystem.addObserver(&drawDataManager);
...
2. **Scene/Node Updates**
   - Apply any changes to scene node transforms (e.g., via `scene.setLocalTransform`).
3. **System Synchronization and Transform Propagation**
   - Call `scene.finalizeForRendering()` to perform both final transform propagation (propagating local-to-world transforms for all dirty nodes and their descendants) **and** notify all registered observers (such as CullingSystem, DrawDataManager, etc.) of node transform changes. This ensures all systems are synchronized exactly once per frame, just before rendering.
   - **Note:** You should **not** call `scene.propogateTransforms()` independently per frame; `finalizeForRendering()` handles both propagation and observer notification in a single, efficient step.
   - **Call `camera.update(deltaTime)` after `finalizeForRendering()`, so that if the camera is attached to a scene node, it uses up-to-date world transforms.**
...
while (!shouldQuit) {
    ...
    scene.finalizeForRendering(); // Propagates transforms, notifies observers, and clears changed nodes
...
- **Scene and Renderer**: The scene module is responsible for transform propagation and tracking changed nodes. The renderer and other systems synchronize their data after transforms are updated and observer notifications are sent via `finalizeForRendering()`.
- **System Data Ownership**: Each system owns its per-node data and is responsible for updating it in response to scene changes.
- **Explicit Synchronization**: No system should assume transforms or per-node data are up to date except immediately after the observer notification phase in the main loop, which is now handled by `finalizeForRendering()`.
